#!/usr/bin/env python3
"""
Quick test for batch filter processing with real model.

This script performs a quick test with a small dataset to verify
...
def quick_test():
    """Run a quick test with 5 documents."""
    print("🚀 Quick Batch Processing Test")
    print("="*50)
    
    # Configure model
    model = LM(model="openrouter/google/gemini-2.5-flash")
    ...
    # Small test dataset
    docs = [
        {"text": "This product is absolutely amazing and exceeded my expectations!"},
        {"text": "Terrible product, complete waste of money and time."},
        {"text": "The product arrived on time and works as described."},
        {"text": "I love this service, it's fantastic and works perfectly."},
        {"text": "Poor quality, disappointed with this purchase."},
    ]
    ...
    try:
        ...
        print(f"Cost improvement: {((individual_cost - batch_cost) / individual_cost * 100):.1f}%" if individual_cost > 0 else "N/A")
        print(f"Token improvement: {((individual_tokens - batch_tokens) / individual_tokens * 100):.1f}%" if individual_tokens > 0 else "N/A")
