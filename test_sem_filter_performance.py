#!/usr/bin/env python3
"""
Performance benchmark script for sem_filter sync vs async comparison.

This script generates datasets of different scales and compares the performance
of synchronous vs asynchronous sem_filter operations. It provides detailed metrics
...
class SemFilterPerformanceBenchmark:
    """Performance benchmark class for sem_filter operations."""
    
    def __init__(self, output_dir: str = "sem_filter_benchmark_results", iterations: int = 1) -> None:
        """
    ...
    def _initialize_realtime_files(self) -> None:
        ...
        csv_headers = [
            ...
            'max_thread_workers', 'duration', 'memory_used_mb', 
        ...
        with open(self.progress_log_path, 'w', encoding='utf-8') as f:
            f.write(f"Sem Filter Performance Benchmark Progress Log\n")
    ...
    def generate_dataset(self, size: int, name: str) -> List[Dict[str, Any]]:
        """
        ...
        Returns:
            List[Dict[str, Any]]: A list of documents with the specified number of items.
        """
        categories = [
    ...
    def run_benchmark_test(
        ...
        try:
            # Run the filtering
            if use_async:
                result = asyncio.run(sem_filter_async(
                    dataset,
                    self.lm,
                    "Is this document about technology or science topics?",
                    default=True,
                    use_async=use_async,
            ...
            # Verify results
            success = (
                hasattr(result, 'outputs') and
                isinstance(result.outputs, list) and
                len(result.outputs) > 0 and
                all(isinstance(output, bool) for output in result.outputs)
            )
    ...
    def run_comprehensive_benchmark(self) -> None:
        ...
        print("Starting comprehensive sem_filter performance benchmark...")
        ...
        self.total_tests = len(dataset_sizes) * len(concurrency_configs) * 2 * self.iterations
        ...
        for size in dataset_sizes:
            ...
            for max_concurrent_batches, max_thread_workers in concurrency_configs:
                ...
                for iteration in range(self.iterations):
                    current_test += 1
                    test_name = f"sync_{size}_{max_concurrent_batches}_{max_thread_workers}_iter{iteration}"
                ...
                for iteration in range(self.iterations):
                    current_test += 1
                    test_name = f"async_{size}_{max_concurrent_batches}_{max_thread_workers}_iter{iteration}"
    ...
    def generate_performance_report(self) -> None:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("# sem_filter Performance Benchmark Report\n\n")
            ...
            f.write("- Use async mode for datasets with 25+ documents\n")
            f.write("- Start with 4 concurrent batches and 8 thread workers\n")
            f.write("- Increase concurrency for larger datasets (100+ documents)\n")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run sem_filter performance benchmark")
    parser.add_argument(
        "--output-dir", 
        default="sem_filter_benchmark_results",
        help="Directory to save benchmark results (default: sem_filter_benchmark_results)"
    )
    parser.add_argument(
        "--iterations",
        type=int,
